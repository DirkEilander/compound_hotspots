...
@click.option('--reducer', type=click.Choice(['mean', 'sum']), default='mean',
...
def nc_extract(in_fns, out_fn, extract_fn, var_name, dim='id',
    ...
    if points:
        ...
        nc_region_ts(in_fns, out_fn, mapping, var_name=var_name, mask=mask, reducer=reducer,
            nc_xdim=xdim, nc_ydim=ydim, nc_tdim=tdim, chunksize=chunksize,
...
def nc_region_ts(in_nc_fns, out_nc_fn, region_gdf, var_name,
    ...
    # use nc convention oder for dimensions
