...
def resample_day(yr):
    ...
    ds_wl = xr.open_dataset(fn_wl)
    ds_wl_clim = xr.open_dataset(fn_wl_clim)
    # combine
    # ds_wl_day = ds_wl['waterlevel'].resample(time='D')    
    # ds_wl_clim_day = ds_wl_clim['waterlevel'].resample(time='D')
    # ds_tide_day = ds_tide['tide'].resample(time='D')
    # ds_surge_day = ds_surge['surge'].resample(time='D')
    # ds_surge_clim_day = (ds_wl_clim['waterlevel'] - ds_tide['tide']).resample(time='D')
    # instant
    time_index = pd.date_range(start=datetime(yr, 1, 1), end=datetime(yr, 12, 31, 0, 0), freq='1D')
    ds_wl_day_inst = ds_wl['waterlevel'].sel(time=time_index, method='nearest')
    ds_wl_clim_day_inst = ds_wl_clim['waterlevel'].sel(time=time_index, method='nearest')
    assert np.all((ds_wl_day_inst-ds_wl_clim_day_inst).isel(time=slice(0,-1)).values != 0)
    # save combined waterlevel
    dim = ('time', 'stations')
    ds_out = xr.merge([
        xr.DataArray(ds_wl_day_inst, dims=dim, name='wl_inst'),
        xr.DataArray(ds_wl_clim_day_inst, dims=dim, name='wl_clim_inst'),
        xr.DataArray(ds_wl_day_inst-ds_wl_clim_day_inst, dims=dim, name='surge_inst'),

        # xr.DataArray(ds_wl_day.max('time'), dims=dim, name='wl_max'),
        # xr.DataArray(ds_wl_day.mean('time'), dims=dim, name='wl_mean'),
        # xr.DataArray(ds_wl_clim_day.max('time'), dims=dim, name='wl_clim_max'),
        # xr.DataArray(ds_wl_clim_day.mean('time'), dims=dim, name='wl_clim_mean'),
        # xr.DataArray(ds_tide_day.max('time'), dims=dim, name='tide_max'),
        # xr.DataArray(ds_tide_day.min('time'), dims=dim, name='tide_min'),
        # xr.DataArray(ds_surge_day.max('time'), dims=dim, name='surge_max'),
        # xr.DataArray(ds_surge_day.mean('time'), dims=dim, name='surge_mean'),
        # xr.DataArray(ds_surge_clim_day.mean('time'), dims=dim, name='surge_clim_mean'),
        ds_tide[['station_id', 'station_name', 'stations']]
        ]).chunk({'time':-1})
    # ds_out['skewsurge'] = ds_out['wl_max'] - ds_out['tide_max'] 
