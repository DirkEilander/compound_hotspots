...
@click.option('--force-dtype', help='overwrite ctl file based dtype (numpy dtype format)')
@click.option('--force-endian', type=click.Choice(['little', 'big']),
                help='overwrite little / big endian setting from ctl file')
@click.option('--force-overwrite', help='force overwrite of existing output geotiff file', is_flag=True)
@click.option('--cell-center/--cell_corner', default=True)
def bin_2_tif(in_fn, fn_out=None, epsg=4326, force_overwrite=False,
    ...
    for fn in fns:
        ...
        try:
            ...
            click.echo("Data file not found for {}.".format(fn))
...
def ctl_parser(fn):
    ...
    for i in range(nvar):
        ...
        if ctl['vars'][var_name]['units'] in vars_unity_dtype.keys():
            ...
            raise ValueError("unknown dtype: {}".format(ctl['vars'][var_name]['units']))
